...
@section('content')
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-5 gap-3">
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('students.create') }}"
                    class="btn btn-primary shadow-lg border-0 px-4 py-2 rounded-3 fw-bold">
                ...
                <div class="card border-0 shadow-sm rounded-4 p-3 bg-white h-100">
                    <div class="d-flex justify-content-between">
                        <div>
                            ...
                            <span class="text-danger x-small fw-bold"><i class="fas fa-exclamation-triangle"></i> Needs
                                Attention</span>
                        </div>
                ...
                <div class="row align-items-center g-3">
                    <div class="col-md-4">
                        <div class="search-box position-relative">
                            ...
                            <input type="text" class="form-control ps-5 border-light bg-light rounded-pill py-2"
    ...
    <div class="btn-group shadow-sm rounded-pill overflow-hidden">
        ...
        <a href="{{ route('students.index') }}"
           class="btn btn-white border-0 px-4 {{ request('status') == null ? 'active' : '' }}">
            All
        </a>

        <a href="{{ route('students.index', ['status' => 'active']) }}"
           class="btn btn-white border-0 px-4 {{ request('status') == 'active' ? 'active' : '' }}">
            Active
        </a>

        <a href="{{ route('students.index', ['status' => 'graduated']) }}"
           class="btn btn-white border-0 px-4 {{ request('status') == 'graduated' ? 'active' : '' }}">
            Graduated
        </a>

        <a href="{{ route('students.index', ['status' => 'blacklist']) }}"
           class="btn btn-white border-0 px-4 {{ request('status') == 'blacklist' ? 'active' : '' }}">
            Blacklist
        </a>
                                ...
                                <td class="ps-4">
                                    <div class="d-flex align-items-center">
                                        <div class="position-relative me-3">
                                            ...
                                            @endphp
                                            <img src="{{ $profileImage }}" class="rounded-circle shadow-sm object-fit-cover"
                                                width="48" height="48" alt="{{ $student->name }}"
                                        ...
                                        <div>
                                            ...
                                            <span class="text-muted x-small">REG:
                                                {{ date('Y') }}-0{{ $student->id }}</span>
                                        </div>
                                    </div>
                                </td>
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end border-0 shadow-lg rounded-3">
                                            <li><a class="dropdown-item py-2"
                                                    href="{{ route('students.show', $student->id) }}"><i
                                                        class="fas fa-id-badge me-2 text-primary"></i> Profile Card</a></li>
                                            <li><a class="dropdown-item py-2"
                                                    href="{{ route('students.ledger', $student->id) }}"><i
                                                        class="fas fa-file-invoice me-2 text-warning"></i> View Ledger</a>
                                            </li>
                                            <li>
                                                <hr class="dropdown-divider">
                                            </li>
                                            <li>
                                                <form action="{{ route('students.terminate', $student->id) }}"
                                                    method="POST"
                                                    onsubmit="return confirm('Are you sure you want to terminate this student?');">
                                                    @csrf
                                                    @method('DELETE')

                                                    <button type="submit" class="dropdown-item py-2 text-danger">
                                                        <i class="fas fa-user-minus me-2"></i> Terminate
                                                    </button>
                                                </form>
                                            </li>
